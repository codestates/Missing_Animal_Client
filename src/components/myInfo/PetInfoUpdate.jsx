...
function PetInfoUpdate({ toggleToModifyPetInfo, handleToModifyPetInfo, isLogged, token, id, title, petname, description, species, sex, missingDate, area, reward, createdAt, petsImages }) {
    ...
    if (!state.isLogged) {
        return (
            <div className="petInfoModify">Pet Info를 수정하려면 로그인이 필요합니다.</div>
        )
    }
    ...
    return (
        <div className="myInfo">
            ...
            <input type='text' placeholder='et Name' value={state.petname}
                onChange={(event) => setState((prevState) => ({
                    ...prevState,
            ...
            <button className="requestModifyPetInfoButton"
                onClick={(event) => {
